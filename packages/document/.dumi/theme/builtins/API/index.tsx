...
const CompositeType: FC<PropertySchema> = (prop: PropertySchema) => {
  ...
  return prop.source?.[0] && themeConfig.sourceLink ? (
    <a className="dumi-default-api-link" href={intl.formatMessage({ id: '$internal.api.sourceLink' }, { ...prop.source[0] })} target="_blank" rel="noreferrer">
      {prop.className}
    </a>
  ) : (
    prop.className
...
const HANDLERS = {
  ...
  reference(prop: Extract<PropertySchema, { type: 'reference' }>): ReactNode {
    ...
    return (
      <>
        <a className="dumi-default-api-link" href={prop.externalUrl} target="_blank" rel="noreferrer">
          {prop.name}
        </a>
        {params.length ? (
          <>
...
const APIRelease: FC<{ name: string; info: string[] }> = ({ name, info }) => {
  ...
  const modiferText = modifer === 'since' ? `${text}+` : intl.formatMessage({ id: `api.component.release.${modifer}` });
  return (
    <span className="dumi-default-api-release">
      <span className="dumi-default-api-release-name" data-release={modifer}>
        {name}
      </span>
      <span {...modiferProps}>{modifer === 'deprecated' ? modiferText : <Badge type="info">{modiferText}</Badge>}</span>
    </span>
  );
};
...
const API: FC<{
  ...
  console.log(frontmatter, components);
